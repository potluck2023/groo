    ...
    var body: some View {
        HStack(spacing: 0) {
            ...
            Button {
                ...
                Image(HomeType.bookshelf.icon)
                    .foregroundColor(iconColor(.bookshelf))
                    .padding(EdgeInsets(top: 11, leading: 24, bottom: 11, trailing: 23.5))
                    .background(backgroundColor(.bookshelf))
                    .cornerRadius(30, corners: [.topRight, .bottomRight])
            }
        }
        .modifier(ColorBorder(Color(hex: 0xEFEFEF), cornerRadius: 30))
        .background(
